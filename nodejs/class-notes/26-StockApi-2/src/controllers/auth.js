...
module.exports = {
  login: async (req, res) => {
    ...
    if (!((req.body.email || req.body.username) && req.body.password))
      throw new CustomError("Please enter username/email and password.");

    const user = await User.findOne({
      $or: [{ username: req.body.username }, { email: req.body.email }],
    });

    if (!user && user.password !== passwordEncrypt(req.body.password))
      throw new CustomError("Wrong email/username or password");
    ...
    const refreshToken = jwt.sign({ _id }, process.env.REFRESH_KEY, {
      expiresIn: "1d",
    });
